(()=>{
  ...
  function upgradeIngredientPickers(root=document){
    root.querySelectorAll('input[data-add-ingredient-name]').forEach(input=>{
      ...
      [...input.attributes].forEach(a=>{if(a.name!=='list'&&a.name!=='type'&&a.name!=='placeholder')sel.setAttribute(a.name,a.value)});
      sel.innerHTML=options();
      sel.dataset.upgraded='1';
      input.replaceWith(sel);
    });
    const builder=document.getElementById('newRecipeIngredientSearch');
    if(builder&&builder.tagName==='INPUT'&&!builder.dataset.upgraded){
      const sel=document.createElement('select');
      sel.id=builder.id;
      sel.dataset.upgraded='1';
      sel.innerHTML=options();
      builder.replaceWith(sel);
    }
  }
  ...
  function ensureStyles(){
    ...
    s.textContent=`
      ...
      .food-editor-modal{position:fixed;inset:0;background:rgba(0,0,0,.48);z-index:9999;display:flex;align-items:flex-start;justify-content:center;padding:5vh 16px;overflow:auto}
      .food-editor-panel{width:min(720px,100%);background:var(--card,#fff);color:inherit;border-radius:16px;padding:18px;box-shadow:0 18px 60px rgba(0,0,0,.25)}
      ...
      .food-editor-panel input,.food-editor-panel select,.food-editor-panel textarea{width:100%;box-sizing:border-box;padding:10px;border:1px solid #bbb;border-radius:9px;background:inherit;color:inherit}
  ...
  function openRecipeEditor(id){
    ...
    const wrap=modal('Edit recipe',body,(box,close)=>{
      ...
      const ingredients=[...box.querySelectorAll('.editor-ingredient-row')].map(row=>({ingredientId:row.querySelector('select').value,amount:Number(row.querySelector('input').value||0)})).filter(x=>x.ingredientId);
  ...
  function ingredientRowHtml(id,amount){
    const ing=ingredientList.find(i=>i.id===id);
    return `<div class="editor-ingredient-row"><select>${'<option value="">Choose ingredient…</option>'+sortedIngredients().map(i=>`<option value="${escapeHtml(i.id)}" ${i.id===id?'selected':''}>${escapeHtml(i.name)}</option>`).join('')}</select><input type="number" min="0" step="0.1" value="${Number(amount||0)}"><button type="button" data-editor-remove-row>×</button></div>`;
  }
