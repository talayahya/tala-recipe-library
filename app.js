...
function addEntryToDay(label, macros){ const date = getSelectedDate(); if(!dailyLog[date]) dailyLog[date] = {entries:[]}; dailyLog[date].entries.push({id:`entry-${Date.now()}-${Math.random().toString(16).slice(2)}`, label, macros, createdAt:new Date().toISOString()}); saveDailyLog(); renderDailyTracker(); showToast('Added'); }
...
function handleRecipeGridClick(event){
  ...
  if(toggleButton){ const card = toggleButton.closest('.recipe-card'); const collapsed = card.classList.toggle('collapsed'); toggleButton.textContent = collapsed ? '＋' : '−'; toggleButton.setAttribute('aria-label', collapsed ? 'Expand recipe' : 'Minimize recipe'); return; }
