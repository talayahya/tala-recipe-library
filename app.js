...
const openRecipesKey = 'recipeLibraryOpenRecipes';
...
function currentRecipeItems(recipe){
  return (recipe.ingredients || []).map((item, index) => {
    const input = document.querySelector(`[data-recipe="${recipe.id}"][data-row-index="${index}"]`);
    return {ingredientId:item.ingredientId, amount:Number(input?.value ?? item.amount ?? 0)};
  });
}
function recipeMacros(recipe){ return recipeMacrosFromIngredients(currentRecipeItems(recipe)); }
function itemDetails(items){
  return (items || []).map(item => {
    const ing = ingredientMap.get(item.ingredientId);
    return {name: ing?.name || item.ingredientId, amount:Number(item.amount || 0), unit: ingredientUnitLabel(ing)};
  });
}
...
function renderTagChips(recipe){ return getRecipeTags(recipe).map(t => `<span class="tag">${escapeHTML(t)} <button type="button" class="tag-x" data-remove-tag="${escapeHTML(recipe.id)}" data-tag="${escapeHTML(t)}">×</button></span>`).join(''); }
...
function renderRecipes(){
  ...
  matchingRecipes.forEach(recipe => {
    const isOpen = openRecipes.has(recipe.id);
    const card = document.createElement('article');
    card.className = `card recipe-card ${isOpen ? '' : 'collapsed'}`;
...
function addEntryToDay(label, macros, details=[]){ const date = getSelectedDate(); if(!dailyLog[date]) dailyLog[date] = {entries:[]}; dailyLog[date].entries.push({id:`entry-${Date.now()}-${Math.random().toString(16).slice(2)}`, label, macros, details, createdAt:new Date().toISOString()}); saveDailyLog(); renderDailyTracker(); showToast('Added'); }
...
function renderEntryDetails(entry){
  ...
  return `<div class="entry-details ${openEntries.has(entry.id) ? '' : 'hidden'}" id="details-${escapeHTML(entry.id)}">${entry.details.map(d => `<div><span>${escapeHTML(d.name)}</span><strong>${fmt(d.amount)} ${escapeHTML(d.unit)}</strong></div>`).join('')}</div>`;
}
function renderDailyTracker(){
  ...
  document.getElementById('dailyEntries').innerHTML = entries.length ? entries.map(e => `<div class="entry-card"><div class="entry"><span>${escapeHTML(e.label)}</span><span>${fmt(e.macros.calories)} kcal · P ${fmt(e.macros.protein)} · C ${fmt(e.macros.carbs)} · F ${fmt(e.macros.fat)} · Fi ${fmt(e.macros.fibre)}</span>${e.details?.length ? `<button type="button" data-toggle-entry="${escapeHTML(e.id)}">${openEntries.has(e.id) ? '−' : '＋'}</button>` : '<span></span>'}<button type="button" data-delete-entry="${escapeHTML(e.id)}">×</button></div>${renderEntryDetails(e)}</div>`).join('') : '<p class="seasoning">No entries logged for this day yet.</p>';
...
function handleRecipeGridClick(event){
  ...
  if(toggleButton){ const recipeId = toggleButton.dataset.recipeId; const card = toggleButton.closest('.recipe-card'); const nowOpen = card.classList.toggle('collapsed') === false; if(nowOpen) openRecipes.add(recipeId); else openRecipes.delete(recipeId); saveOpenRecipes(); toggleButton.textContent = nowOpen ? '−' : '＋'; return; }
  if(addButton){ const recipeId = addButton.dataset.addTag; const recipe = recipes.find(r => r.id === recipeId); const input = document.querySelector(`[data-tag-input="${recipeId}"]`); const newTag = normalizeTag(input.value); if(recipe && newTag){ setRecipeTags(recipeId, [...getRecipeTags(recipe), newTag]); openRecipes.add(recipeId); saveOpenRecipes(); refreshTagFilter(); renderRecipes(); } }
  if(removeButton){ const recipeId = removeButton.dataset.removeTag; const recipe = recipes.find(r => r.id === recipeId); if(recipe){ setRecipeTags(recipeId, getRecipeTags(recipe).filter(t => t !== removeButton.dataset.tag)); openRecipes.add(recipeId); saveOpenRecipes(); refreshTagFilter(); renderRecipes(); } }
  if(addIngredientButton){ const recipeId = addIngredientButton.dataset.addIngredient; const recipe = recipes.find(r => r.id === recipeId); const nameInput = document.querySelector(`[data-add-ingredient-name="${recipeId}"]`); const amountInput = document.querySelector(`[data-add-ingredient-amount="${recipeId}"]`); const ing = findIngredientByInput(nameInput.value); if(recipe && ing){ recipe.ingredients.push({ingredientId: ing.id, amount: Number(amountInput.value || 0)}); persistRecipe(recipe); openRecipes.add(recipeId); saveOpenRecipes(); renderRecipes(); showToast('Added'); } else alert('Ingredient not found. Try typing the ingredient exactly as it appears in the list.'); }
  if(removeIngredientButton){ const recipeId = removeIngredientButton.dataset.removeIngredient; const recipe = recipes.find(r => r.id === recipeId); const index = Number(removeIngredientButton.dataset.index); if(recipe && recipe.ingredients[index]){ recipe.ingredients.splice(index, 1); persistRecipe(recipe); openRecipes.add(recipeId); saveOpenRecipes(); renderRecipes(); } }
  if(addRecipeToDayButton){ const recipe = recipes.find(r => r.id === addRecipeToDayButton.dataset.addRecipeToDay); if(recipe){ const items = currentRecipeItems(recipe); addEntryToDay(recipe.name, recipeMacrosFromIngredients(items), itemDetails(items)); } }
  if(deleteCustomButton){ const recipeId = deleteCustomButton.dataset.deleteCustomRecipe; customRecipes = customRecipes.filter(r => r.id !== recipeId); delete customTags[recipeId]; openRecipes.delete(recipeId); saveOpenRecipes(); saveCustomTags(); saveCustomRecipes(); refreshAllRecipes(); refreshTagFilter(); renderRecipes(); }
}
function setupEvents(){
  ...
  document.getElementById('dailyEntries').addEventListener('click', event => { const toggle = event.target.closest('[data-toggle-entry]'); if(toggle){ const id = toggle.dataset.toggleEntry; if(openEntries.has(id)) openEntries.delete(id); else openEntries.add(id); renderDailyTracker(); return; } const btn = event.target.closest('[data-delete-entry]'); if(!btn) return; const date = getSelectedDate(); dailyLog[date].entries = getEntriesForDate(date).filter(e => e.id !== btn.dataset.deleteEntry); openEntries.delete(btn.dataset.deleteEntry); saveDailyLog(); renderDailyTracker(); });
  document.getElementById('addBuilderIngredient').addEventListener('click', () => { const ing = findIngredientByInput(document.getElementById('newRecipeIngredientSearch').value); if(!ing){ alert('Ingredient not found. Try typing the ingredient exactly as it appears in the list.'); return; } builderIngredients.push({ingredientId: ing.id, amount: Number(document.getElementById('newRecipeIngredientAmount').value || 0)}); document.getElementById('newRecipeIngredientSearch').value = ''; renderBuilder(); showToast('Added'); });
